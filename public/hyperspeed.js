...
class Hyperspeed {
  constructor(container, effectOptions = {}) {
    ...
    this.effectOptions = {
      ...
      colors: {
        roadColor: 0x030303,
        islandColor: 0x040404,
        background: 0x000000,
        shoulderLines: 0x666666,
        brokenLines: 0x666666,
        leftCars: [0x8B1A75, 0x3D2A5C, 0x7A1E6B],
        rightCars: [0x015255, 0x083246, 0x1F2A33],
        sticks: 0x015255,
      },
      ...effectOptions
    };
  ...
  init() {
    ...
    this.initDistortions();
    this.initApp(lightsContainer);
  }
  ...
  initApp(container) {
    ...
    options.distortion = this.distortions[options.distortion];

    this.appRef = new HyperspeedApp(container, options);
    this.appRef.loadAssets().then(() => this.appRef.init());
  }
...
class HyperspeedApp {
  ...
  update(delta) {
    ...
    let fovChange = this.lerp(this.camera.fov, this.fovTarget, lerpPercentage);
    if (fovChange !== 0) {
  ...
  tick() {
    ...
    if (this.resizeRendererToDisplaySize(this.renderer, this.setSize)) {
  ...
  resizeRendererToDisplaySize(renderer, setSize) {
...
class CarLights {
  ...
  init() {
    ...
    let material = new THREE.ShaderMaterial({
      fragmentShader: this.getCarLightsFragment(),
      vertexShader: this.getCarLightsVertex(),
      transparent: true,
...
class LightsSticks {
  ...
  init() {
    ...
    const material = new THREE.ShaderMaterial({
      fragmentShader: this.getSideSticksFragment(),
      vertexShader: this.getSideSticksVertex(),
...
class Road {
  ...
  createPlane(side, width, isRoad) {
    ...
    const material = new THREE.ShaderMaterial({
      fragmentShader: isRoad ? this.getRoadFragment() : this.getIslandFragment(),
      vertexShader: this.getRoadVertex(),
      side: THREE.DoubleSide,
      uniforms: Object.assign(
        uniforms,
        this.webgl.fogUniforms,
